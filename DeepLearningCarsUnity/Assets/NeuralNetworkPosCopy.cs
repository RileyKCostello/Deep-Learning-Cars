...
public class NeuralNetworkPosCopy : MonoBehaviour
{
    public float tempID;
    ...
    public struct Layer
    {
        ...
        public void RandomBias()
        {
            ...
            {
                //Random.value - 0.5 might return negative value? idk so i put the math max function
                biases[i] = Mathf.Max(0, (Random.value - 0.5f) * 2);
            }
        }
        ...
        public void RandomWeight()
        {
            for(int i = 0; i < weights.entries.Length; i++)
            {
                weights.entries[i] = Random.value;
            }
        }
    ...
    public void CopyLayers(NeuralNetwork toCopy)
    {
        ...
        //TODO: Find a way to get rid of this monstrosity
        /*
        for (int i = 0; i < input.weights.entries.Length; i++)
        {
            input.weights.entries[i] = toCopy.input.weights.entries[i];
        }
        for (int i = 0; i < input.biases.Length; i++)
        {
            input.biases[i] = toCopy.input.biases[i];
        }

        for (int i = 0; i < h1.weights.entries.Length; i++)
        {
            h1.weights.entries[i] = toCopy.h1.weights.entries[i];
        }
        for (int i = 0; i < h1.biases.Length; i++)
        {
            h1.biases[i] = toCopy.h1.biases[i];
        }

        for (int i = 0; i < h2.weights.entries.Length; i++)
        {
            h2.weights.entries[i] = toCopy.h2.weights.entries[i];
        }
        for (int i = 0; i < h2.biases.Length; i++)
        {
            h2.biases[i] = toCopy.h2.biases[i];
        }
        */
    }
    ...
    {
        //CalculateOutputs moved to the CarMovement Script
        //CalculateOutputs();
        CalculateID();
    }
    ...
    public void Start()
    {
        //tempID = input.weights[1, 1] + h1.weights[1, 1] + h2.weights[1, 1];
        tempID = CalculateID();
    }
